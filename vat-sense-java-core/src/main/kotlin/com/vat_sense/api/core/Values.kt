...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { VatSenseInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw VatSenseInvalidDataException("`$name` is not set")
            is JsonNull -> throw VatSenseInvalidDataException("`$name` is null")
            else -> throw VatSenseInvalidDataException("`$name` is invalid, received $this")
        }
